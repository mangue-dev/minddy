# Agent setup for `minddy`

This is a deepsec scanning workspace. Project `minddy` was just registered
(target: `..`). Setup is incomplete — `data/minddy/INFO.md`
...
2. **Fill in `data/minddy/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id minddy
pnpm deepsec process --project-id minddy
```
