...
| GitLab Webhook | Token **specific to the repository**, resolved by `project.id` and compared with `timingSafeEqual` (this is not an HMAC signature). The payload's `path_with_namespace` must be registered under the same id before any name-scoped handler runs. Token encryption is required; otherwise the route responds 503. The historical global token is migration-only, is unavailable to unknown repositories, and is revoked for a repository as soon as its dedicated ciphertext is stored. An absent or unrecognized token responds 401. Anti-replay uses `X-Gitlab-Event-UUID`. |
