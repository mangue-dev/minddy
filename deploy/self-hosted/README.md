...
and sends authenticated requests only to the internal `minddy` service.
`CRON_SECRET` is generated by the installer and the scheduler starts by default.

`agent-runner` creates a restricted Docker container for each server-side agent
run. It is part of both reference profiles and uses
`AGENT_EXECUTION_BACKEND=self-hosted`; no Vercel credentials or desktop machine
are required. Its control API remains on the private network. Because creating
containers requires Docker socket access, treat the runner as privileged
infrastructure and never publish its port or secret. Individual agent sandboxes
do not receive the socket or the private Supabase network.
