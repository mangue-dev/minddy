...
The board can be embedded in a product flow through its public URL. Published project pages can also be shown as tabs beside the public board. Optional SSO pre-identifies visitors with a short-lived signed token. Server-to-server feedback ingestion uses a project integration key, not the public board. Numo can help configure and triage the board, but public replies are only sent when explicitly requested.
