...
Routines are scheduled instructions that run on a project and use the owner's AI budget. A routine's instructions can mention issues, pages, or objectives, and the agent receives that context each time the routine runs. Personal statistics give each user a view of completed issues, pace, and time spent, with breakdowns by project, category, and objective. The task notebook is a private, optional scratchpad for quick notes and checkboxes before they become issues. A notebook note can be promoted into an issue when it grows into work that the project should track.
