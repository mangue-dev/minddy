      ...
      for (const purpose of ["content", "blind_index"]) {
        ...
        for (const version of [1, 2]) {
          ...
          sql(database, `INSERT INTO public.envelope_data_keys
            ...
            VALUES('${kind}','${scopeId}','${purpose}',${version},'${wrapped}',${version === 2});`);
        }
      }
    }
    const before = JSON.parse(sql(database, "SELECT json_agg(k ORDER BY scope_kind,scope_id,purpose,version) FROM public.envelope_data_keys k;"));
...
`, { mode: 0o700 });
    ...
    assert.deepEqual(await runRootRewrap(["--verify-root"], env), { applied: false, count: 12 });
    assert.deepEqual(await runRootRewrap([], env), { applied: false, count: 12 });
    assert.deepEqual(await runRootRewrap(["--apply", "--confirm-writes-stopped"], env), { applied: true, count: 12 });
    const after = JSON.parse(sql(database, "SELECT json_agg(k ORDER BY scope_kind,scope_id,purpose,version) FROM public.envelope_data_keys k;"));
