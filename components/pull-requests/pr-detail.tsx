 ...
 *
 * What the run still decides: the “Generated by Numo” badge and the “and
 * relaunch Numo”. Without a run, all other gestures remain — merge, refuse,
 * review, comments are forging gestures, not agent gestures.
 *
 * Three gestures in the action bar: **Review** (approve / request
 * changes / comment, with the box “and restart Numo” which only exists
 * at minddy), **Refuse** (closes the PR), **Merge** (split button: the
 * main method is squash, chevron gives the other methods than
 * forge accepte).
 */
...
export function PrDetail({
  ...
  // Squash remains the main action (that's what minddy has been doing since MIN-46);
  // the chevron only offers what the forge actually accepts.
  ...
  const submitReview = async () => {
    ...
    // without saying anything is not (and both forges refuse an empty body).
  ...
  return (
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2 px-4 py-3 md:px-6">
        ...
        {/* Approvals: “n approvals”, and the mention of blocking next to the
            Merge button. No “n/N” — the N comes from the protection of
            branch, which costs an out-of-scope GitHub permission. */}
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {mergeMethods.map((m) => (
                        <DropdownMenuItem
                          key={m}
                          // On red CI, only the explicit entry “merge
                          // anyway » passes: choosing a method should not
                          // become a quiet bypass of the guardrail.
      ...
      >
        <div className="mx-auto flex max-w-3xl flex-col gap-6 py-6">
          ...
          {/* CI Checks — under the header, before the feed: this is what we look at
              before deciding to merge. */}
          {!loading ? (
            <ChecksBanner checks={checks} error={checksError} provider={item.provider} />
          ) : null}

          {/* The only place to say “what git account are you operating under”
