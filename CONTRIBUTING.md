...
and creates or updates its pull request with the given title and description;
repeat `-m` for several description paragraphs. The description must be a real,
complete text — what was done, why, and anything a reviewer needs to know —
never a bare `Signed-off-by` trailer: the sign-off belongs to the commits. Run
without arguments, the command falls back to filling the pull request from the
commit messages. The done command refuses to run until
