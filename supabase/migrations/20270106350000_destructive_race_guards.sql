-- Keep destructive decisions and their writes in the same transaction. A
-- caller may use an earlier read to find candidates, but none of these
-- functions trusts that read when it removes or replaces durable state.
