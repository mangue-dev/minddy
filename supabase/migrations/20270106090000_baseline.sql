...
-- minddy — consolidated schema baseline (MIN-379).
--
-- Generated by `supabase migration squash --local` after complete application of
-- the previous history. Do not edit by hand: create a migration afterwards
-- this file. Initial data and Storage buckets live
-- respectively in `20270106091000_initial_data.sql` and in the bootstrap,
-- because they are not part of a PostgreSQL schema dump.
...
begin
  ...
  return null;
exception when others then
  -- A broadcast failure should NEVER cause a channel's progress to fail.
  return null;
end;
$$;
...
declare
  ...
  old_rec jsonb := null;
begin
  -- The body and its two derivatives exit the payload. `-` on a jsonb
  -- has no effect if the key does not exist: the function survives a column
  ...
  return null;
exception when others then
  -- A failed broadcast NEVER brings down the writing: live is the
  -- comfort, writing the page is the normal path.
  return null;
end;
$$;
...
begin
  ...
  return null;
exception when others then
  -- A failed broadcast should NEVER bring down a webhook: ingestion
  -- of a PR is the normal path, direct is only comfort.
  return null;
end;
$$;
...
COMMENT ON COLUMN "public"."agent_runs"."deployment_url" IS 'Deployment affinity (MIN-165): null = production or local (drained by the prod cron), otherwise the EXACT VERCEL_URL of the preview deployment which created the run — only this deployment claims it, over its entire duration. We stamp the deployment and not the branch: a run continues with the code that launched it, even if we push the branch between two chunks.';



COMMENT ON COLUMN "public"."agent_runs"."provider_key_id" IS 'Revocation identifier of the LLM key issued for this run (OpenRouter hash). Never the secret. Null = no key per run (BYOK, or provisioning not configured).';



COMMENT ON COLUMN "public"."agent_runs"."loop_in_vm" IS 'The loop of this run runs IN the microVM (MIN-224). Frozen on launch from app_config.agent_loop_in_vm_projects: a conversation never changes engine during its life.';
...
COMMENT ON COLUMN "public"."agent_runs"."agent_engine" IS 'The harness that plays this run: ''opencode'' (headless server controlled by our supervisor, MIN-286) for any new run, ''loop'' for runs prior to the switch. Written at creation and never reread elsewhere: each engine rereads ITS memory in the checkpoint, so a conversation never changes engine during its life.';



COMMENT ON COLUMN "public"."agent_runs"."local_exec" IS 'This run runs on the user''s machine (MIN-355), not in a microVM. Written at creation and never reread elsewhere, like loop_in_vm and agent_engine: a conversation does not change environment during its life.';



COMMENT ON COLUMN "public"."agent_runs"."local_exec_gen" IS 'Generation of the local execution lease (MIN-355). The harness token bears this number as a claim; issuing a token increments it, which kills all previous ones. This is the only possible revocation of a self-bearing token, and it is free: it is paid where the run line is already read.';



COMMENT ON COLUMN "public"."agent_runs"."local_worktree" IS 'The local run works in an isolated worktree, created on the machine that executes the tour. Frozen on launch; the path remains local to this machine.';
...
begin
  ...
  -- Opportunistic housekeeping: the cardinality of the table is that of the logins of
  -- forge crusaders, which an attacker can make grow at will. A chance
  -- one hundred per call is enough to keep it going, and costs nothing on the normal route.
  if random() < 0.01 then
...
begin
  if new.objective_id is not null and not exists (
    ...
    raise exception 'The objective must be in the same project';
  end if;
  ...
  if new.duplicate_of_id is not null and not exists (
    ...
    raise exception 'The duplicate ticket must be in the same project';
  end if;
...
begin
  if new.parent_id is not null then
    ...
    if exists (
      ...
      raise exception 'Nesting limited to one level';
    end if;
    if exists (
      select 1 from public.issues p
      where p.id = new.parent_id and p.project_id <> new.project_id
    ) then
      raise exception 'Parent must be in the same project';
    end if;
    if exists (select 1 from public.issues c where c.parent_id = new.id) then
      raise exception 'This issue already has sub-issues';
    end if;
  end if;
  return new;
end;
...
begin
  if new.project_id is distinct from old.project_id then
    -- 42501 = insufficient_privilege: PostgREST makes it 403, not 500.
...
CREATE OR REPLACE FUNCTION "public"."get_admin_user_totals"("p_tz" "text" DEFAULT 'UTC'::"text") RETURNS "jsonb"
  ...
  -- Joining on `accounts` suddenly removes traces of internal accounts
  -- AND those of deleted accounts (the `ai_usage` ledger keeps the attribution).
  activity as (
    select ra.user_id, ra.day
    from raw_activity ra
    join accounts acc on acc.id = ra.user_id
  ),
  -- “Public” projects: those of a counted account. The tickets follow.
  live_projects as (
    select p.id
...
CREATE OR REPLACE FUNCTION "public"."get_admin_users_overview"("p_search" "text" DEFAULT NULL::"text", "p_limit" integer DEFAULT 25, "p_offset" integer DEFAULT 0) RETURNS TABLE("user_id" "uuid", "email" "text", "meta" "jsonb", "created_at" timestamp with time zone, "last_sign_in_at" timestamp with time zone, "email_confirmed_at" timestamp with time zone, "is_internal" boolean, "projects_owned" bigint, "projects_member" bigint, "issues_accessible" bigint, "issues_created" bigint, "last_activity_at" timestamp with time zone, "spent_month" numeric, "ai_calls" bigint, "reset_at" timestamp with time zone, "total_count" bigint)
  ...
  select
    ...
    -- GREATEST ignores NULLs in Postgres: an account with no activity returns
    -- simply his last connection (or NULL if he has never connected).
  ...
  left join lateral (
    select
      -- LIVING projects: a soft-deleted project no longer counts, neither for the
      -- admin view nor for the onboarding signal “create your first project”.
      (select count(*)
      ...
      -- ACCESSIBLE tickets (owned projects + joined projects): this is the
      -- signal that the onboarding reads, and the “number of tickets” that the admin
      ...
      -- Tickets written in his hand, wherever they are: his real contribution.
      (select count(*)
         from public.issues i
  ...
  left join lateral (
    -- CALENDAR MONTH expenditure, gross. The expense actually counted by the
    -- budget (Stripe window + reset watermark) is resolved side
    -- route by `getUserUsage` — it depends on the billing cycle.
    select
  ...
  -- The table is a REGISTER since 20261105: one line per reset.
  -- A direct join would therefore multiply the account line by the number of
  -- actions taken — the page would start repeating the accounts and lying about
  -- its total. What matters here is the MOST RECENT, and that alone.
  left join lateral (
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        -- null (and not 0) when no rate is known: “not yet
        -- convertible” and “zero euro” do not mix up on the screen.
...
CREATE OR REPLACE FUNCTION "public"."get_ai_usage_stats"("p_since" timestamp with time zone DEFAULT ("now"() - '30 days'::interval)) RETURNS "jsonb"
  ...
  -- one record per run (a run_id always belongs to a single feature)
  runs as (
    select
      run_id,
...
CREATE OR REPLACE FUNCTION "public"."get_cycle_stats"("p_tz" "text" DEFAULT 'UTC'::"text") RETURNS "jsonb"
  ...
  -- 1. Cadence: gap (days) between completion and deadline. due_date is
  -- a timestamptz (migration 20260707120000): we bring it back, like
  -- completed_at, on its calendar date in p_tz so that date - date gives
  -- an integer (not an interval).
  ...
  -- 2. Tickets per cycle: average over cycles already started.
  started_cycles as (
    select c.id
  ...
  -- 3. Completion time per effort (“cycle time”: 1st in_progress → done).
  first_started as (
...
CREATE OR REPLACE FUNCTION "public"."get_user_usage_history"("p_user_id" "uuid", "p_since" timestamp with time zone, "p_features" "text"[] DEFAULT NULL::"text"[], "p_limit" integer DEFAULT 25, "p_offset" integer DEFAULT 0) RETURNS "jsonb"
  ...
  with runs as (
    select
      u.run_id,
      -- min(): in a mixed agent run, 'agent_code' < 'sandbox_compute' —
      -- the run keeps the type that the user recognizes.
...
begin
  ...
  -- Fixed a jump upwards if the target is itself tombstoned
  -- (flattening guarantees depth ≤ 1).
  ...
  -- Locks ordered by id (anti-deadlock).
  ...
  -- A post promoted to issue cannot be absorbed (it remains a valid target).
  if v_dup.issue_id is not null then raise exception 'feedback_merge_dup_promoted'; end if;

  -- Votes present on both sides: deduplicated (deleted on the duplicate side).
  ...
  -- The rest migrates (the trigger maintains the two counters).
  with moved as (
  ...
  -- Flattening: everything that pointed to the duplicate points back to the target.
  with rp as (
  ...
  -- Duplicate suggestions become obsolete (not restored at
  -- undo: the analyzer regenerates them).
  ...
  with dormant as (
    select u.id
     ...
     where u.created_at < p_before
       ...
       -- Protects the merge undo: `feedback_merge_events.payload` keeps
       -- uuid of identities that `undo_feedback_merge` reinserts into
       -- `feedback_votes`. An identity cited by an undefeated merger would make
       -- fail undo on foreign key — it waits its turn.
       and not exists (
...
declare
  ...
  new_turn_id uuid;
begin
  -- Waking up after a terminal state = new natural demand. A
  ...
  if old.status in ('completed', 'failed', 'canceled') and new.status = 'queued' then
    ...
    -- Run end-of-turn: /steer can insert his message during the run
    -- is still `running`, then see `completed` and requeue it. THE
    -- capture trigger then temporarily attached it to the previous turn.
    -- Any message not yet consumed is precisely the request that must
    -- open this new turn: we repair its connection in the same
    -- transaction as terminal transition -> queued.
...
begin
  ...
  -- The facets having been removed (MIN-50), only the post merges are undone.
  ...
  -- Lock of both posts, ordered by id.
  ...
  if v_state is distinct from v_ev.canonical_id then
    -- The canonical has since been merged (the duplicate has been re-pointed): undo
    -- LIFO only — undo the most recent merge first.
    raise exception 'feedback_undo_stale';
  end if;

  -- Moved votes: return if they still exist (a post-merge unvote
  -- applies to the merged concept and is not resurrected).
  update public.feedback_votes set post_id = v_ev.dup_id
   where post_id = v_ev.canonical_id and user_id = any(v_moved);

  -- Deduplicated votes: Their vote on the duplicate was independent.
  insert into public.feedback_votes (post_id, user_id)
  select v_ev.dup_id, u from unnest(v_dropped) as u
  on conflict do nothing;

  -- Untombstone without re-entering the analysis queue.
  update public.feedback_posts
  ...
  -- Anti-recurrence memory: the pair will no longer be re-merged or re-suggested.
...
COMMENT ON TABLE "public"."agent_conversation_contexts" IS 'Optional resources and multiple data in context. None decides the visibility or capabilities of the agent.';
...
COMMENT ON TABLE "public"."agent_conversations" IS 'Durable identity of a conversation with the code agent. Its context, its executions and its visibility are independent dimensions.';
...
COMMENT ON TABLE "public"."agent_run_journal" IS 'MIN-286 — opencode session event log, append. Reassembled in the order of `id` to replay the session on a fresh microVM. The pointer (current session + seq cursor) lives in agent_runs.checkpoint.opencode.';
...
COMMENT ON COLUMN "public"."ai_usage"."billed_reason" IS 'Why is this line imputed to user_id (MIN-131, MIN-150). null = row before column.';
...
COMMENT ON TABLE "public"."assistant_active_conversation" IS 'The open Numo conversation for this user (MIN-353). One line per user: the scope (project or global) is carried by the conversation itself, not by this pointer. No line = no open conversation (the panel starts from a blank screen).';
...
COMMENT ON COLUMN "public"."attachments"."kind" IS 'file = object in bucket attachments; link = external URL (MIN-184); page = project wiki page (MIN-275).';
...
COMMENT ON COLUMN "public"."attachments"."page_id" IS 'Project page referenced; null excluding page type resource (MIN-275).';
...
COMMENT ON COLUMN "public"."billing_accounts"."admin_override_expires_at" IS 'End of the plan offered by an admin (null = no limit). Once passed, the override is ignored at resolution.';
...
COMMENT ON TABLE "public"."feedback_sso_replays" IS 'Board SSO tokens already consumed (MIN-345). The primary key refuses replay; lines are purged past expires_at.';
...
COMMENT ON TABLE "public"."mfa_recovery_codes" IS '2FA recovery codes (MIN-132, hardened to MIN-347). Salted scrypt fingerprint by code: scrypt$<N>$<sel hex>$<empreinte hex>. Role only service (RLS without policy). A consumed code disables the account''s 2FA.';
...
COMMENT ON COLUMN "public"."pages"."parent_block_removed" IS 'MIN-272 — a subpage block was removed from the parent body by trashing this page; the restoration puts it back.';
...
COMMENT ON COLUMN "public"."projects"."orb_seed" IS 'Orb seed generated. Null = never restarted, the reader falls back to projects.id.';
...
COMMENT ON TABLE "public"."share_unlock_attempts" IS 'Failed to unlock a protected share (MIN-347). Persistent counter — the in-memory counter restarted from zero each time it was deployed. Opportunistic purge beyond the window. Role only service (RLS without policy).';
...
COMMENT ON COLUMN "public"."user_ai_keys"."validated_at" IS 'Time when the provider recognized the key (MIN-344). null = not validated: the key does not raise any ceiling.';
...
COMMENT ON COLUMN "public"."view_shares"."page_id" IS 'Wiki page published as read (MIN-283); null on a view share.';



COMMENT ON COLUMN "public"."view_shares"."include_children" IS 'The entire branch leaves with the page (MIN-283) — explicit choice upon publication.';
...
-- `supabase migration squash` does not export Realtime schema policies.
-- They are nevertheless part of the distributed application diagram: copy them here
-- keeps a blank bootstrap aligned with instances that have applied MIN-351.
