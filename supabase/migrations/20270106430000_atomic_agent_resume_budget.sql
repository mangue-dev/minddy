-- Re-reserve managed-AI budget when an inactive conversation is resumed.
-- Completed rows do not hold reservations, so the status transition and the
-- new account reservation must share the same account-scoped transaction lock.
...
BEGIN
  ...
  IF v_run.id IS NULL OR v_run.created_by IS DISTINCT FROM p_user_id
     ...
     OR v_run.status NOT IN ('completed', 'canceled')
