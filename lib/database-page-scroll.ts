...
export function bindDatabasePageScroll(viewport: HTMLElement): () => void {
  ...
  const scroll = (delta: number) => {
    viewport.scrollLeft = Math.max(
      0,
      Math.min(viewport.scrollWidth - viewport.clientWidth, viewport.scrollLeft + delta),
    );
  };
  ...
  let gesture: { x: number; y: number; lastX: number; axis: "pending" | "x" | "y" } | null = null;
  const touchStart = (event: TouchEvent) => {
    const touch = event.touches.length === 1 && overflows() ? event.touches[0] : null;
    gesture = touch ? { x: touch.clientX, y: touch.clientY, lastX: touch.clientX, axis: "pending" } : null;
  };
  ...
  const touchEnd = () => { gesture = null; };

  surface.addEventListener("wheel", wheel, { capture: true, passive: false });
  surface.addEventListener("touchstart", touchStart, { capture: true, passive: true });
  surface.addEventListener("touchmove", touchMove, { capture: true, passive: false });
