...
async function fetchMyCycleApi(): Promise<MyCycleResponse> {
  ...
  return (data ?? { enabled: false, current: null }) as MyCycleResponse;
}

/**
 * The user's current cycle (MIN-32) for surfaces outside the global board —
 * today the project boards' right-click "Add to cycle". Skips the request
 * entirely while cycles are disabled (the pref lives in auth metadata, so the
 * gate is known client-side).
 */
