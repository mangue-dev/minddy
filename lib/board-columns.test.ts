...
describe("buildBoardColumns", () => {
  it("groups in status order and sorts each populated column", () => {
    const columns = buildBoardColumns(
      ...
      (_columnIssues) => (a, b) => a.position - b.position
    );
  ...
  it("does not mutate the source issue order", () => {
    ...
    buildBoardColumns(statuses, issues, (_columnIssues) => (a, b) => a.position - b.position);
  ...
  it("reuses an unaffected column across issue updates", () => {
    ...
    const first = build(statuses, [todo, doing], (_columnIssues) => (a, b) => a.position - b.position);
    const second = build(
      statuses,
