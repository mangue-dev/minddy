...
// prompt moves that issue to "in_progress". Stored in the account's Supabase
// auth `user_metadata.prompt_copy_auto_start` — same mechanism as
// `auto_assign_created` / `numo_default_status`. Enabled by default: the
// preference is only off when explicitly set to `false`. Server-safe: imports
// only the enum type, no React/lucide.
