...
/** Au-delà de ce cumul stdout+stderr, la sortie complète est déposée sur disque. */
export const RUN_COMMAND_SPILL_THRESHOLD = 8000;
...
/** La sortie mérite-t-elle d'être déposée en entier dans la sandbox ? */
export function spillsToDisk(o: Pick<CommandOutput, "stdout" | "stderr">): boolean {
  return o.stdout.length + o.stderr.length > RUN_COMMAND_SPILL_THRESHOLD;
}
...
function buildResult(
  ...
  if (!fullOutputPath || !truncated) {
    return { exitCode: o.exitCode, stdout, stderr };
  }
  return {
    exitCode: o.exitCode,
    stdout,
