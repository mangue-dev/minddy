...
import type { CommentTable } from "../comment-store";

/** Only these converted repositories are eligible; snapshots are not inferred from the policy inventory. */
export async function backfillCommentsBatch(table: CommentTable, limit = 50, signal?: AbortSignal) {
  ...
  const repository: RowMigrationRepository = {
    async scan(batchLimit) {
      ...
      if (error) throw new Error("Unable to scan comment migration");
      ...
      for (const row of data ?? []) {
        if (typeof row.id !== "string" || typeof row.project_id !== "string" ||
          ...
          throw new Error("Invalid comment migration metadata");
        }
        const { data: marked, error: attemptError } = await service.rpc("migrate_comment_ciphertext", {
          p_table: table, p_id: row.id, p_project_id: row.project_id,
          p_revision: row.encryption_revision, p_previous_version: row.encryption_version,
        });
        if (attemptError) throw new Error("Unable to record comment migration attempt");
        if (!marked) continue;
    ...
    async compareAndSwap(candidate, replacement) {
      const { data, error } = await service.rpc("migrate_comment_ciphertext", {
        p_table: table, p_id: candidate.row.id, p_project_id: candidate.context.scope.id,
        p_revision: Number(candidate.revision), p_previous_version: candidate.row.encryption_version,
        p_encryption_version: replacement.encryption_version, p_encrypted_content: replacement.encrypted_content,
      });
      if (error) throw new Error("Unable to commit comment migration");
      return data === true;
    },
  };
